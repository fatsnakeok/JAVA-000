...
public class RedisRepository {
    ...
    public void setExpire(final String key, final Object value, final long time) {
        redisTemplate.execute((RedisCallback<Long>) connection -> {
            ...
            byte[] values = getRedisValueSerializer().serialize(value);
    ...
    public void setExpire(final String key, final long time) {
        redisTemplate.execute((RedisCallback<Long>) connection -> {
            ...
            byte[] keys = serializer.serialize(key);
            connection.expire(keys, time);
            LOGGER.info("[redisTemplate redis]设置 缓存  url:{} ========缓存时间为{}秒", key, time);
            return 1L;
        });
    }
     ...
     * @Description 查看key对应的过期时间（默认为毫秒）
     * @Date 2018/3/12 下午2:37
     */
    public Long getKeyExpire(final String key) {
        return getKeyExpire(key, TimeUnit.MILLISECONDS);
    }
     ...
     * @param timeUnit 时间参数
     * @return java.lang.Long
     * @Description 查看key对应的过期时间
     * @Date 2018/3/12 下午2:36
     */
    public Long getKeyExpire(final String key, TimeUnit timeUnit) {
        return redisTemplate.execute((RedisCallback<Long>) connection -> {
            RedisSerializer<String> redisSerializer = getRedisSerializer();
            return connection.pTtl(redisSerializer.serialize(key), timeUnit);
        });
    }
    ...
    public <T> T get(final String key, Class<T> elementType) {
        T resultStr = redisTemplate.execute((RedisCallback<T>) connection -> {
            ...
            byte[] keys = serializer.serialize(key);
            byte[] values = connection.get(keys);
            return (T) getRedisValueSerializer().deserialize(values);
        });
        LOGGER.info("[redisTemplate redis]取出 缓存  url:{} ", key);
        return resultStr;
    }
    ...
    public boolean exists(final String key) {
        return redisTemplate.execute((RedisCallback<Boolean>) connection ->
            connection.exists(key.getBytes(DEFAULT_CHARSET)));
    }
